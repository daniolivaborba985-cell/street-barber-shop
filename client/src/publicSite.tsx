...
function AssistantPage({ barber }) {
  ...
  const times = useMemo(() => Array.from({ length: 23 }, (_, index) => { const total = 9 * 60 + index * 30; return `${String(Math.floor(total / 60)).padStart(2, '0')}:${String(total % 60).padStart(2, '0')}` }), [])
