...
function ClubRoulette({ mode, onSpin, spinning, reward, onLogin }) {
  ...
  const activate = () => { if (!spinning) (mode === 'vip' ? onSpin : onLogin)() }
  const handleKeyDown = (event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); activate() } }
  return <section id="roleta" className="club-roulette-section"><div className="club-roulette-copy"><p className="eyebrow">ROULETA VIP</p><h2>Um giro.<br /><em>Uma vantagem.</em></h2><p>Um giro por ciclo. A recompensa real é definida pelo Clube.</p><span className="club-roulette-rule">1 giro · ciclo ativo</span></div><div className="club-wheel-stage"><span className="club-wheel-pointer" aria-hidden="true">▼</span><div className={spinning ? 'club-wheel is-spinning' : 'club-wheel'} role="button" tabIndex={spinning ? -1 : 0} aria-disabled={spinning} aria-label={mode === 'vip' ? 'Girar roleta de descontos' : 'Entrar na Área VIP para girar a roleta'} onClick={activate} onKeyDown={handleKeyDown}>{segments.map((segment, index) => <span key={`${segment}-${index}`} style={{ '--segment-index': index }}>{segment}</span>)}</div><div className="club-wheel-result" role="status">{reward ? `Recompensa: ${reward.name}` : 'Pronto para girar'}</div></div><button type="button" className="primary club-wheel-button" onClick={activate} disabled={spinning}>{mode === 'vip' ? (spinning ? 'Girando…' : 'Girar agora') : 'Entrar na Área VIP'} <Sparkles size={16} /></button></section>
}
...
function ClubPage() {
  ...
  useEffect(() => { if (!selectedBarberSlug && barberCatalog.data?.length) setSelectedBarberSlug(barberCatalog.data[0].slug) }, [barberCatalog.data, selectedBarberSlug])
