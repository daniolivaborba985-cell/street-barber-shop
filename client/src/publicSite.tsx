...
function ClubRoulette({ mode, onSpin, spinning, reward, onLogin }) {
  ...
  return <section id="roleta" className="club-roulette-section"><div className="club-roulette-copy"><p className="eyebrow">ROULETA VIP</p><h2>Um giro.<br /><em>Uma vantagem.</em></h2><p>Um giro por ciclo. A recompensa real é definida pelo Clube.</p><span className="club-roulette-rule">1 giro · ciclo ativo</span></div><div className="club-wheel-stage"><span className="club-wheel-pointer" aria-hidden="true">▼</span><div className={spinning ? 'club-wheel is-spinning' : 'club-wheel'} aria-label="Roleta de descontos do Clube">{segments.map((segment, index) => <span key={`${segment}-${index}`} style={{ '--segment-index': index }}>{segment}</span>)}</div><div className="club-wheel-result" role="status">{reward ? `Recompensa: ${reward.name}` : 'Pronto para girar'}</div></div><button type="button" className="primary club-wheel-button" onClick={mode === 'vip' ? onSpin : onLogin} disabled={spinning}>{mode === 'vip' ? (spinning ? 'Girando…' : 'Girar agora') : 'Entrar na Área VIP'} <Sparkles size={16} /></button></section>
}
...
function ClubPage() {
  ...
  return <div className="app inner-page club-app"><ClubHeader /><main className="club-page club-page-live"><div className="club-live-hero"><div className="club-hero-copy"><p className="eyebrow">STREET CLUB · CLIENTE VIP</p><h1>O próximo nível<br /><em>da sua presença.</em></h1><p className="page-lead">Uma comunidade de benefícios para manter seu visual em dia, aproveitar vantagens e fazer parte da Street Barber Shop além da cadeira.</p><div className="club-actions"><button type="button" className="primary" onClick={() => setMode('contract')}>Escolher meu plano <ArrowRight size={17} /></button><button type="button" className="secondary" onClick={() => { setFeedback(''); setMode('login') }}><Crown size={16} /> Acessar área VIP</button></div></div><div className="club-live-mark"><Crown size={30} /><span>STREET</span><small>VIP COMMUNITY</small><b>MEMBER / 001</b></div></div><div className="club-hero-proof"><span><strong>05</strong> planos oficiais</span><span><strong>30</strong> dias por ciclo</span><span><strong>VIP</strong> comunidade</span></div><ClubEditorialSections partners={partners} raffles={raffles} mode={mode} onVip={() => { setFeedback(''); setMode('login') }} onJoinRaffle={(raffleId) => joinRaffle.mutate({ raffleId }, { onSuccess: () => setFeedback('Você entrou nesta campanha mensal.'), onError: (error) => setFeedback(error.message) })} joinRafflePending={joinRaffle.isPending} onFeedback={setFeedback} /><ClubRoulette mode={mode} onSpin={doSpin} spinning={spin.isPending} reward={rouletteReward} onLogin={() => { setFeedback(''); setMode('login') }} /><ClubPlanShowcase catalog={catalog} selectedSlug={selectedSlug} onSelect={(slug) => { setSelectedSlug(slug); window.history.replaceState({}, '', `/clube?plano=${encodeURIComponent(slug)}`) }} onContract={() => setMode('contract')} />
    ...
    {mode === 'contract' && <section id="vip" className="club-panel"><div className="club-panel-head"><div><p className="eyebrow">ATIVAÇÃO DO CLUBE</p><h2>Escolha seu plano<br /><em>e mantenha o ritmo.</em></h2></div><button type="button" className="panel-close" aria-label="Fechar contratação" onClick={() => setMode('explore')}><X size={18} /></button></div><div className="club-plan-selector">{catalog.map((plan) => <button type="button" className={`club-plan-option ${selectedSlug === plan.slug ? 'selected' : ''}`} onClick={() => setSelectedSlug(plan.slug)} key={plan.slug}><span>{plan.name}</span><strong>{plan.price}</strong><small>{plan.details.slice(0, 2).join(' · ')}</small></button>)}</div><form className="club-form" onSubmit={submitRequest}><div className="club-form-grid"><label>Nome completo<input required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="Seu nome" /></label><label>Telefone<input required value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} placeholder="(49) 99999-9999" /></label><label>E-mail<input required type="email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} placeholder="voce@email.com" /></label><label>Forma de pagamento<select value={form.paymentMethod} onChange={(event) => setForm({ ...form, paymentMethod: event.target.value })}><option value="pix">PIX</option><option value="card">Cartão</option></select></label></div><p className="club-form-note">O VIP ativa após a confirmação do pagamento.</p><button className="primary" type="submit" disabled={request.isPending || checkoutRequest.isPending}>{request.isPending ? 'Registrando…' : `Continuar com ${selectedPlan?.name || 'o plano'}`} <ArrowRight size={16} /></button></form></section>}
