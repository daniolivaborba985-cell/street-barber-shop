...
import { CalendarDays, Check, Clock3, Copy, ExternalLink, LayoutDashboard, Loader2, LockKeyhole, LogIn, Menu, Plus, ShieldAlert, Users, WalletCards, X } from "lucide-react";
...
type BarberFilter = "all" | "luan" | "bruno" | "kaua";
...
const CHATBOT_BARBERS = [{ slug: "luan", name: "Luan", assistant: "Assistente Luan" }, { slug: "bruno", name: "Bruno", assistant: "Assistente Bruno" }, { slug: "kaua", name: "Kauã", assistant: "Assistente Kauã" }] as const;
const chatbotUrl = (slug: string) => `${typeof window === "undefined" ? "" : window.location.origin}/assistentes/${slug}`;

function ChatbotLinksCard({ user }: { user: NonNullable<ReturnType<typeof useAuth>["user"]> }) {
  const [copied, setCopied] = useState<string | null>(null);
  const visible = user.role === "barber" ? CHATBOT_BARBERS.filter((barber) => barber.slug === (["luan", "bruno", "kaua"] as const)[(user.barberId ?? 3) - 1]) : CHATBOT_BARBERS;
  const copyLink = async (slug: string) => { await navigator.clipboard?.writeText(chatbotUrl(slug)); setCopied(slug); window.setTimeout(() => setCopied(null), 1800); };
  return <section className="admin-panel admin-chatbot-panel"><div className="admin-panel-head"><div><p className="admin-eyebrow">LINKS DE AGENDAMENTO</p><h2>Chatbots da equipe</h2><p>Links prontos para compartilhar com seus clientes.</p></div><ExternalLink size={18} /></div><div className="admin-chatbot-grid">{visible.map((barber) => <article className="admin-chatbot-card" key={barber.slug}><div><span className="admin-chatbot-kicker">{barber.assistant}</span><strong>{barber.name}</strong><code>/assistentes/{barber.slug}</code></div><div className="admin-chatbot-actions"><Button size="sm" onClick={() => copyLink(barber.slug)}><Copy size={14} /> {copied === barber.slug ? "Copiado" : "Copiar link"}</Button><a className="admin-chatbot-open" href={chatbotUrl(barber.slug)} target="_blank" rel="noreferrer"><ExternalLink size={14} /> Abrir</a></div></article>)}</div></section>;
}
...
function DashboardTab({ user }: { user: NonNullable<ReturnType<typeof useAuth>["user"]> }) {
  ...
  return <>
    ...
    <section className="admin-metrics" aria-label="Indicadores principais">
      ...
      {user.role !== "barbearia" && <Metric icon={WalletCards} label={user.role === "barber" ? "Seu faturamento" : "Faturamento"} value={money(data?.metrics.revenueCents)} tone="admin-metric-violet" />}
      <Metric icon={X} label="Cancelamentos" value={data?.metrics.cancelled ?? 0} tone="admin-metric-muted" />
    </section>
    <ChatbotLinksCard user={user} />
...
function AgendaTab({ user }: { user: NonNullable<ReturnType<typeof useAuth>["user"]> }) {
  ...
  const [form, setForm] = useState({ barberSlug: user.role === "barber" ? (["luan", "bruno", "kaua"] as const)[user.barberId ? user.barberId - 1 : 2] : "luan" as "luan" | "bruno" | "kaua", name: "", phone: "", email: "", serviceSlugs: ["corte"], appointmentDate: new Date().toISOString().slice(0, 10), startTime: "09:00" });
  const [selectedBarber, setSelectedBarber] = useState<BarberFilter>(user.role === "barber" ? (["luan", "bruno", "kaua"] as const)[user.barberId ? user.barberId - 1 : 2] : "all");
  const rows = query.data ?? [];
  const filteredRows = selectedBarber === "all" ? rows : rows.filter((row) => row.barberSlug === selectedBarber);
  ...
  return <><header className="admin-heading"><div><p className="admin-eyebrow">CALENDÁRIO ÚNICO</p><h1>Agenda da barbearia</h1><p>Os mesmos agendamentos ocupam os horários do site e do painel.</p></div></header><AgendaCalendar user={user} rows={rows} selectedBarber={selectedBarber} onBarberChange={(value) => { setSelectedBarber(value); if (value !== "all") setForm((current) => ({ ...current, barberSlug: value })); }} /><section className="admin-panel"><div className="admin-panel-head"><div><p className="admin-eyebrow">NOVO AGENDAMENTO</p><h2>Agendar pelo painel</h2></div></div><form className="admin-block-form" onSubmit={submit}><label>Barbeiro<select value={form.barberSlug} onChange={(event) => setForm({ ...form, barberSlug: event.target.value as "luan" | "bruno" | "kaua" })} disabled={user.role === "barber"}>{user.role === "barber" ? <option value={form.barberSlug}>Kauã</option> : <><option value="luan">Luan</option><option value="bruno">Bruno</option><option value="kaua">Kauã</option></>}</select></label><label>Serviço<select value={form.serviceSlugs[0]} onChange={(event) => setForm({ ...form, serviceSlugs: [event.target.value] })}><option value="corte">Corte</option><option value="barba">Barba</option><option value="sobrancelha">Sobrancelha</option><option value="limpeza-de-pele">Limpeza de pele</option><option value="bigode-e-cavanhaque">Bigode e cavanhaque</option></select></label><label>Data<Input type="date" value={form.appointmentDate} onChange={(event) => setForm({ ...form, appointmentDate: event.target.value })} /></label><label>Horário<Input type="time" step="1800" value={form.startTime} onChange={(event) => setForm({ ...form, startTime: event.target.value })} /></label><label>Cliente<Input required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="Nome completo" /></label><label>Telefone<Input required value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} placeholder="(00) 00000-0000" /></label><label>E-mail<Input required type="email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} placeholder="cliente@email.com" /></label><Button type="submit" disabled={create.isPending}><Plus size={16} /> {create.isPending ? "Salvando…" : "Criar agendamento"}</Button></form>{create.error && <p className="admin-error admin-form-error">{create.error.message}</p>}</section><section className="admin-panel"><div className="admin-panel-head"><div><p className="admin-eyebrow">ATENDIMENTOS</p><h2>Agenda compartilhada</h2></div><span className="admin-live-note"><span /> Banco conectado</span></div>{query.isLoading ? <div className="admin-empty"><Loader2 className="animate-spin" /> Carregando agenda…</div> : <AppointmentTable rows={filteredRows} onConfirm={(id) => update.mutate({ id, status: "confirmed" })} onCancel={(id) => update.mutate({ id, status: "cancelled" })} onReschedule={promptReschedule} />}</section><div className="admin-note"><strong>Operações seguras</strong><span>Confirmação e cancelamento registram histórico. O reagendamento mantém o registro anterior e valida conflitos antes de salvar.</span><div className="admin-actions">{filteredRows[0] && filteredRows[0].status !== "confirmed" && <Button size="sm" onClick={() => update.mutate({ id: filteredRows[0].id, status: "confirmed" })}><Check size={14} /> Confirmar próximo</Button>}{filteredRows[0] && filteredRows[0].status !== "cancelled" && <Button size="sm" variant="outline" onClick={() => update.mutate({ id: filteredRows[0].id, status: "cancelled" })}><X size={14} /> Cancelar próximo</Button>}</div></div></>;
}
...
function ReportsTab({ user }: { user: NonNullable<ReturnType<typeof useAuth>["user"]> }) {
  ...
  const [selectedBarber, setSelectedBarber] = useState<BarberFilter>(user.role === "barber" ? (["luan", "bruno", "kaua"] as const)[user.barberId ? user.barberId - 1 : 2] : "all");
  const query = trpc.admin.reports.useQuery({ fromDate, toDate, barberSlug: selectedBarber === "all" ? undefined : selectedBarber }, { enabled: user.role === "admin" || user.role === "barber" });
  const clubReport = trpc.admin.clubReport.useQuery(undefined, { enabled: user.role === "admin" || user.role === "barber" });
  return <><header className="admin-heading"><div><p className="admin-eyebrow">PERFORMANCE · {user.role === "barber" ? "SEU ESCOPO" : "CONSOLIDADO"}</p><h1>Relatórios</h1><p>{user.role === "barber" ? "Seu faturamento e seus indicadores, sem acesso aos dados dos outros barbeiros." : "Alterne entre cada barbeiro ou consulte o consolidado da operação."}</p></div><div className="admin-report-controls"><label className="admin-filter-label">Ver resultados de<select aria-label="Escolher barbeiro no relatório" value={selectedBarber} onChange={(event) => setSelectedBarber(event.target.value as BarberFilter)} disabled={user.role === "barber"}><option value="all">Todos os barbeiros</option><option value="luan">Luan</option><option value="bruno">Bruno</option><option value="kaua">Kauã</option></select></label><div className="admin-date-filter"><Input type="date" value={fromDate} onChange={(event) => setFromDate(event.target.value)} /><span>até</span><Input type="date" value={toDate} onChange={(event) => setToDate(event.target.value)} /></div></div></header>{query.isLoading ? <div className="admin-empty"><Loader2 className="animate-spin" /> Calculando relatório…</div> : <><section className="admin-metrics"><Metric icon={WalletCards} label="Faturamento total" value={money(query.data?.revenueCents)} tone="admin-metric-gold" /><Metric icon={CalendarDays} label="Atendimentos" value={query.data?.appointments ?? 0} /><Metric icon={WalletCards} label="Planos pagos" value={money(query.data?.planPaymentsRevenueCents)} tone="admin-metric-violet" /><Metric icon={X} label="Cancelamentos" value={query.data?.cancellations ?? 0} tone="admin-metric-muted" /></section>{clubReport.data && <section className="admin-panel admin-club-report"><div className="admin-panel-head"><div><p className="admin-eyebrow">STREET CLUB · FATURAMENTO</p><h2>Planos confirmados</h2><p>As compras aprovadas aparecem separadas dos atendimentos e são atribuídas ao barbeiro escolhido no checkout.</p></div></div><div className="admin-more-data-grid"><article><span>Receita de planos no período</span><strong>{money(query.data?.planPaymentsRevenueCents)}</strong></article><article><span>Planos pagos no período</span><strong>{query.data?.planPaymentsCount ?? 0}</strong></article><article><span>Clientes VIP ativos</span><strong>{clubReport.data.activeMembers}</strong></article><article><span>Cadastros pendentes</span><strong>{clubReport.data.pendingMembers}</strong></article><article><span>Ciclos ativos</span><strong>{clubReport.data.activeCycles}</strong></article><article><span>Pagamentos pendentes</span><strong>{clubReport.data.pendingPayments}</strong></article><article><span>Benefícios consumidos</span><strong>{clubReport.data.consumedBenefits}</strong></article><article><span>Sorteios abertos</span><strong>{clubReport.data.openRaffles}</strong></article><article><span>Giros registrados</span><strong>{clubReport.data.rouletteSpins}</strong></article></div>{query.data?.planPayments?.length ? <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Cliente</th><th>Plano</th><th>Barbeiro responsável</th><th>Método</th><th>Valor</th><th>Confirmado em</th></tr></thead><tbody>{query.data.planPayments.map((payment) => <tr key={payment.id}><td><strong>{payment.customerName}</strong><small>{payment.customerEmail}</small></td><td>{payment.planName}</td><td>{payment.barberName}</td><td>{payment.method === "card" ? "Cartão" : "PIX"}</td><td>{money(payment.amountCents)}</td><td>{payment.paidAt ? new Date(payment.paidAt).toLocaleDateString("pt-BR") : "—"}</td></tr>)}</tbody></table></div> : <div className="admin-empty"><WalletCards size={20} /> Nenhum plano pago neste período.</div>}</section>}<section className="admin-panel"><div className="admin-panel-head"><div><p className="admin-eyebrow">DESEMPENHO POR BARBEIRO</p><h2>Comparativo do período</h2></div></div><div className="admin-report-grid">{query.data?.byBarber.map((barber) => <article className="admin-report-card" key={barber.barberName}><strong>{barber.barberName}</strong><span>{barber.appointments} atendimentos</span><b>{money(barber.revenueCents)}</b><small>{barber.cancellations} cancelamentos · {barber.planPayments} planos ({money(barber.planPaymentsCents)})</small></article>)}</div></section><section className="admin-panel admin-more-data-panel"><div className="admin-panel-head"><div><p className="admin-eyebrow">MAIS DADOS</p><h2>Performance operacional</h2><p>Indicadores conectados aos agendamentos, disponibilidade e bloqueios reais.</p></div></div><div className="admin-more-data-grid">{query.data?.moreData && <><article><span>Clientes únicos</span><strong>{query.data.moreData.uniqueCustomers}</strong></article><article><span>Atendimentos por cliente</span><strong>{query.data.moreData.appointmentsPerCustomer.toFixed(2)}</strong></article><article><span>Performance</span><strong>{query.data.moreData.performancePercent}%</strong></article><article><span>Receita por hora</span><strong>{money(query.data.moreData.revenuePerHourCents)}</strong></article><article><span>Pendências</span><strong>{query.data.moreData.pendingPercent}%</strong></article><article><span>Top serviço</span><strong>{query.data.moreData.topService}</strong></article><article><span>Horas disponíveis</span><strong>{query.data.moreData.availableHours} h</strong></article><article><span>Horas trabalhadas</span><strong>{query.data.moreData.workedHours} h</strong></article><article><span>Tempo ocioso</span><strong>{query.data.moreData.idleHours} h</strong></article><article><span>Agenda fechada</span><strong>{query.data.moreData.closedHours} h</strong></article><article><span>Upsell produto</span><strong>{query.data.moreData.upsellPercent == null ? "Sem dados" : `${query.data.moreData.upsellPercent}%`}</strong></article></>}</div></section></>}</>;
}
