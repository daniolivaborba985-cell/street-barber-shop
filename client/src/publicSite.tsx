...
function ClubHeader() {
  ...
  const closeMenu = () => setMenu(false)
  return <header className="site-header club-site-header"><InternalLink className="brand" href="/#inicio" onClick={closeMenu}><img src="/manus-storage/logo_41568a93.jpeg" alt="Street Barber Shop" /><span>STREET<small>BARBER SHOP</small></span></InternalLink><button type="button" className="club-mobile-toggle" aria-label={menu ? 'Fechar menu do Clube' : 'Abrir menu do Clube'} aria-expanded={menu} onClick={() => setMenu((current) => !current)}>{menu ? <X size={23} /> : <Menu size={23} />}</button><nav className={menu ? 'open' : ''} aria-label="Navegação do Street Barber Clube"><InternalLink href="/clube#o-clube" onClick={closeMenu}>O Clube</InternalLink><a href="/clube#beneficios-premium" onClick={closeMenu}>Benefícios</a><a href="/clube#parceiros" onClick={closeMenu}>Parceiros</a><InternalLink href="/clube#planos-clube" onClick={closeMenu}>Planos</InternalLink><a href="/clube#area-vip" onClick={closeMenu}>Área VIP</a><InternalLink href="/#inicio" className="nav-cta" onClick={closeMenu}>Voltar ao site <ArrowRight size={15} /></InternalLink></nav></header>
}
...
function ClubEditorialSections({ partners, raffles, mode, onVip, onJoinRaffle, joinRafflePending, onFeedback }) {
  return <>
    <section id="o-clube" className="club-section club-manifesto"><div className="club-manifesto-copy"><p className="eyebrow">01 · O CLUBE</p><h2>Você não é apenas<br /><em>cliente.</em></h2><p>Mais presença. Mais benefícios. Um Clube feito para acompanhar sua rotina.</p><div className="club-manifesto-signature"><span>STREET</span><b>BARBER CLUB</b></div></div><div className="club-manifesto-art"><div className="club-art-ring"><Crown size={34} /><span>MEMBER<br />ONLY</span></div><div className="club-art-note">Acesso reservado<br /><strong>à comunidade</strong></div></div></section>
    <section id="beneficios-premium" className="club-section club-benefits-editorial"><div className="club-section-heading compact"><p className="eyebrow">02 · BENEFÍCIOS</p><h2>Presença que<br /><em>volta para você.</em></h2></div><div className="club-benefit-list"><article><span className="club-benefit-number">A</span><div><h3>Seu visual em dia</h3><p>Cortes e barbas no seu ciclo.</p></div></article><article><span className="club-benefit-number">B</span><div><h3>Detalhes que contam</h3><p>Sobrancelha de brinde.</p></div></article><article><span className="club-benefit-number">C</span><div><h3>Fora da cadeira</h3><p>Vantagens fora da cadeira.</p></div></article><article><span className="club-benefit-number">D</span><div><h3>Convites especiais</h3><p>Campanhas e vantagens VIP.</p></div></article></div></section>
    <section id="parceiros" className="club-section club-partners-editorial"><div className="club-section-heading"><p className="eyebrow">03 · QUEM ESTÁ COM A GENTE</p><h2>Marcas que<br /><em>entendem a comunidade.</em></h2><p>Marcas escolhidas para a comunidade.</p></div>{partners.data?.length ? <div className="club-partner-mural">{partners.data.map((partner) => <article className="club-partner-tile" key={partner.id}><div className="partner-logo-lockup">{partner.logoUrl || partner.logo ? <img src={partner.logoUrl || partner.logo} alt={`Logo ${partner.name}`} /> : <span>{partner.name.slice(0, 2).toUpperCase()}</span>}</div><div><p>{partner.category || 'PARCEIRO STREET'}</p><h3>{partner.name}</h3><span>{partner.benefits?.map((benefit) => benefit.title).join(' · ') || partner.description || 'Benefício exclusivo para membros.'}</span></div><button type="button" onClick={() => onFeedback(`O benefício de ${partner.name} estará disponível na sua Área VIP.`)}>Ver benefício <ArrowUpRight size={15} /></button></article>)}</div> : <div className="club-empty-feature"><div className="empty-feature-mark"><Crown size={22} /></div><div><p className="eyebrow">MURAL EM CURADORIA</p><h3>Os primeiros parceiros estão chegando.</h3><p>Este espaço já está pronto para receber logos, categorias e benefícios reais assim que a barbearia cadastrar as marcas no sistema.</p></div></div>}</section>
    <section className="club-section club-sponsors-editorial"><div><p className="eyebrow">04 · PARCEIROS QUE ACREDITAM NO CLUBE</p><h2>Uma rede com<br /><em>presença própria.</em></h2><p>Marcas que acreditam no Clube.</p></div><div className="sponsor-levels"><span>PARCEIRO</span><span>PARCEIRO PREMIUM</span><span>PATROCINADOR OFICIAL</span><small>Nenhum patrocinador cadastrado ainda</small></div></section>
    <section id="como-funciona" className="club-section club-how"><div className="club-section-heading compact"><p className="eyebrow">05 · COMO FUNCIONA</p><h2>Quatro passos.<br /><em>Um novo ritmo.</em></h2></div><div className="club-how-grid"><article><strong>01</strong><h3>Escolha seu plano</h3><p>Escolha seu ritmo.</p></article><article><strong>02</strong><h3>Faça seu cadastro</h3><p>Seu cadastro continua o mesmo.</p></article><article><strong>03</strong><h3>Ative sua assinatura</h3><p>O ciclo começa após a confirmação.</p></article><article><strong>04</strong><h3>Viva como VIP</h3><p>Use. Participe. Aproveite.</p></article></div></section>
    <section id="experiencias" className="club-section club-experiences"><div className="club-experiences-head"><div><p className="eyebrow">06 · EXPERIÊNCIAS</p><h2>Ser VIP tem<br /><em>suas vantagens.</em></h2></div><p>Experiências exclusivas para membros.</p></div>{raffles.data?.length ? <div className="club-raffle-feature-grid">{raffles.data.map((raffle) => <article className="club-raffle-feature" key={raffle.id}><span className="club-raffle-label">CAMPANHA MENSAL</span><h3>{raffle.campaign}</h3><strong>{raffle.prize}</strong><p>Sorteio em {new Date(raffle.drawAt).toLocaleDateString('pt-BR')}</p>{mode === 'vip' && <button type="button" className="club-outline-button" onClick={() => onJoinRaffle(raffle.id)} disabled={joinRafflePending}>Participar da campanha <ArrowRight size={15} /></button>}</article>)}</div> : <div className="club-experience-empty"><Sparkles size={21} /><span>A próxima experiência exclusiva será publicada aqui.</span><small>Campanhas reais aparecerão assim que forem cadastradas pela barbearia.</small></div>}</section>
    <section id="area-vip" className="club-section club-vip-callout"><div className="vip-callout-mark"><Crown size={26} /><span>VIP</span></div><div><p className="eyebrow">07 · ÁREA EXCLUSIVA</p><h2>Já é VIP?</h2><p>Seu plano. Seus benefícios. Seu ritmo.</p></div><button type="button" className="primary" onClick={onVip}>Acessar área VIP <ArrowRight size={17} /></button></section>
  </>
}

function ClubRoulette({ mode, onSpin, spinning, reward, onLogin }) {
  const segments = ['5% OFF', '10% OFF', '15% OFF', 'BRINDE', '20% OFF', 'EXTRA']
  const [previewSpinning, setPreviewSpinning] = useState(false)
  const isSpinning = spinning || previewSpinning
  useEffect(() => { if (!previewSpinning) return; const timer = window.setTimeout(() => { setPreviewSpinning(false); onLogin() }, 2400); return () => window.clearTimeout(timer) }, [previewSpinning])
  const activate = () => { if (isSpinning) return; if (mode === 'vip') onSpin(); else setPreviewSpinning(true) }
  const handleKeyDown = (event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); activate() } }
  return <section id="roleta" className="club-roulette-section"><div className="club-roulette-copy"><p className="eyebrow">ROULETA VIP</p><h2>Um giro.<br /><em>Uma vantagem.</em></h2><p>Um giro por ciclo. A recompensa real é definida pelo Clube.</p><span className="club-roulette-rule">1 giro · ciclo ativo</span></div><div className="club-wheel-stage"><span className="club-wheel-pointer" aria-hidden="true">▼</span><div className={isSpinning ? 'club-wheel is-spinning' : 'club-wheel'} role="button" tabIndex={isSpinning ? -1 : 0} aria-disabled={isSpinning} aria-label={mode === 'vip' ? 'Girar roleta de descontos' : 'Girar roleta e entrar na Área VIP'} onClick={activate} onKeyDown={handleKeyDown}>{segments.map((segment, index) => <span key={`${segment}-${index}`} style={{ '--segment-index': index }}>{segment}</span>)}</div><div className="club-wheel-result" role="status">{previewSpinning ? 'Girando…' : reward ? `Recompensa: ${reward.name}` : mode === 'vip' ? 'Pronto para girar' : 'Gire para conhecer'}</div></div><button type="button" className="primary club-wheel-button" onClick={activate} disabled={isSpinning}>{mode === 'vip' ? (spinning ? 'Girando…' : 'Girar agora') : (previewSpinning ? 'Girando…' : 'Girar e entrar')} <Sparkles size={16} /></button></section>
}
...
function ClubPage() {
  ...
  const login = trpc.club.login.useMutation()
  const logout = trpc.club.logout.useMutation()
  const dashboard = trpc.club.me.useQuery(undefined, { enabled: mode === 'vip', retry: false })
  const myRaffles = trpc.club.myRaffles.useQuery(undefined, { enabled: mode === 'vip', retry: false })
  const partners = trpc.club.partners.useQuery(undefined, { staleTime: 60000 })
  const raffles = trpc.club.raffles.useQuery(undefined, { staleTime: 60000 })
  const joinRaffle = trpc.club.joinRaffle.useMutation()
  const spin = trpc.club.spin.useMutation()
  ...
  const submitRequest = (event) => {
    ...
    request.mutate({ ...form, planSlug: selectedPlan.slug, barberSlug: selectedBarberSlug }, {
      onSuccess: (result) => { setFeedback(`Solicitação registrada para o plano ${selectedPlan.name}. Abrindo o Checkout seguro; o VIP só será ativado após confirmação do pagamento.`); checkoutRequest.mutate({ subscriptionId: result.subscriptionId, email: form.email, phone: form.phone }, { onSuccess: (checkout) => { if (checkout.checkoutUrl) window.open(checkout.checkoutUrl, '_blank', 'noopener,noreferrer'); setLoginForm({ email: form.email, phone: form.phone }); setMode('login') }, onError: (error) => { setLoginForm({ email: form.email, phone: form.phone }); setMode('login'); setFeedback(`A solicitação foi registrada, mas o checkout não pôde ser aberto: ${error.message}`) } }); return result },
  ...
  const submitLogin = (event) => {
    ...
    setFeedback('')
    login.mutate(loginForm, { onSuccess: () => setMode('vip'), onError: (error) => setFeedback(error.message) })
  }
  const doLogout = () => { logout.mutate(undefined, { onSettled: () => { setMode('explore'); setFeedback('Sessão encerrada.') } }) }
  const doSpin = () => { if (!spin.isPending) { setRouletteReward(null); spin.mutate({ idempotencyKey: `roulette-${Date.now()}-${Math.random().toString(36).slice(2)}` }, { onSuccess: (result) => { setRouletteReward(result.spin.reward || null); setFeedback(`Sua recompensa: ${result.spin.reward?.name || 'giro registrado'}.`) }, onError: (error) => setFeedback(error.message) }) } }

  return <div className="app inner-page club-app"><ClubHeader /><main className="club-page club-page-live"><div className="club-live-hero"><div className="club-hero-copy"><p className="eyebrow">STREET CLUB · CLIENTE VIP</p><h1>O próximo nível<br /><em>da sua presença.</em></h1><p className="page-lead">Uma comunidade de benefícios para manter seu visual em dia, aproveitar vantagens e fazer parte da Street Barber Shop além da cadeira.</p><div className="club-actions"><button type="button" className="primary" onClick={() => setMode('contract')}>Escolher meu plano <ArrowRight size={17} /></button><button type="button" className="secondary" onClick={() => { setFeedback(''); setMode('login') }}><Crown size={16} /> Acessar área VIP</button></div></div><div className="club-live-mark"><Crown size={30} /><span>STREET</span><small>VIP COMMUNITY</small><b>MEMBER / 001</b></div></div><div className="club-hero-proof"><span><strong>05</strong> planos oficiais</span><span><strong>30</strong> dias por ciclo</span><span><strong>VIP</strong> comunidade</span></div><ClubEditorialSections partners={partners} raffles={raffles} mode={mode} onVip={() => { setFeedback(''); setMode('login') }} onJoinRaffle={(raffleId) => joinRaffle.mutate({ raffleId }, { onSuccess: () => setFeedback('Você entrou nesta campanha mensal.'), onError: (error) => setFeedback(error.message) })} joinRafflePending={joinRaffle.isPending} onFeedback={setFeedback} /><ClubRoulette mode={mode} onSpin={doSpin} spinning={spin.isPending} reward={rouletteReward} onLogin={() => { setFeedback(''); setMode('login') }} /><ClubPlanShowcase catalog={catalog} selectedSlug={selectedSlug} onSelect={(slug) => { setSelectedSlug(slug); setSelectedBarberSlug(''); window.history.replaceState({}, '', `/clube?plano=${encodeURIComponent(slug)}`) }} onContract={() => setMode('contract')} />
    {feedback && <p className="club-feedback" role="status">{feedback}</p>}
    {mode === 'contract' && <section id="club-active-panel" className="club-panel"><div className="club-panel-head"><div><p className="eyebrow">ATIVAÇÃO DO CLUBE</p><h2>Escolha seu plano<br /><em>e mantenha o ritmo.</em></h2></div><button type="button" className="panel-close" aria-label="Fechar contratação" onClick={() => setMode('explore')}><X size={18} /></button></div><div className="club-plan-selector">{catalog.map((plan) => <button type="button" className={`club-plan-option ${selectedSlug === plan.slug ? 'selected' : ''}`} onClick={() => { setSelectedSlug(plan.slug); setSelectedBarberSlug('') }} key={plan.slug}><span>{plan.name}</span><strong>{plan.price}</strong><small>{plan.details.slice(0, 2).join(' · ')}</small></button>)}</div><form className="club-form" onSubmit={submitRequest}><div className="club-form-grid"><label className="club-barber-field"><span>1 · Escolha seu barbeiro</span><select required value={selectedBarberSlug} onChange={(event) => setSelectedBarberSlug(event.target.value)} disabled={barberCatalog.isLoading || !barberCatalog.data?.length}><option value="" disabled>{barberCatalog.isLoading ? 'Carregando barbeiros…' : 'Escolha seu barbeiro'}</option>{barberCatalog.data?.map((barber) => <option key={barber.slug} value={barber.slug}>{barber.name}</option>)}</select></label><label>Nome completo<input required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="Seu nome" /></label><label>Telefone<input required value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} placeholder="(49) 99999-9999" /></label><label>E-mail<input required type="email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} placeholder="voce@email.com" /></label><label>Forma de pagamento<select value={form.paymentMethod} onChange={(event) => setForm({ ...form, paymentMethod: event.target.value })}><option value="pix">PIX</option><option value="card">Cartão</option></select></label></div><p className="club-form-note">Escolha seu barbeiro. O VIP ativa após a confirmação do pagamento.</p><button className="primary" type="submit" disabled={request.isPending || checkoutRequest.isPending}>{request.isPending ? 'Registrando…' : `Continuar com ${selectedPlan?.name || 'o plano'}`} <ArrowRight size={16} /></button></form></section>}
    {mode === 'login' && <section id="club-active-panel" className="club-panel club-login-panel"><div className="club-panel-head"><div><p className="eyebrow">ÁREA DO CLIENTE VIP</p><h2>Entre para ver<br /><em>seus benefícios.</em></h2></div><button type="button" className="panel-close" aria-label="Fechar login" onClick={() => setMode('explore')}><X size={18} /></button></div><form className="club-form" onSubmit={submitLogin}><div className="club-form-grid"><label>E-mail<input required type="email" value={loginForm.email} onChange={(event) => setLoginForm({ ...loginForm, email: event.target.value })} placeholder="O mesmo e-mail do cadastro" /></label><label>Telefone<input required value={loginForm.phone} onChange={(event) => setLoginForm({ ...loginForm, phone: event.target.value })} placeholder="O mesmo telefone do cadastro" /></label></div><p className="club-form-note">Usamos seu cadastro Street. Nenhum duplicado é criado.</p><button className="primary" type="submit" disabled={login.isPending}>{login.isPending ? 'Entrando…' : 'Acessar minha área'} <ArrowRight size={16} /></button></form></section>}
    {mode === 'vip' && <section id="club-active-panel" className="club-panel club-dashboard-panel"><div className="club-panel-head"><div><p className="eyebrow">ÁREA DO CLIENTE VIP</p><h2>Seu Clube,<br /><em>seu ritmo.</em></h2></div><button type="button" className="secondary" onClick={doLogout}>Sair</button></div>{dashboard.isLoading ? <p className="page-lead">Carregando seus dados…</p> : dashboard.data?.member ? <><div className="vip-status-row"><span className={`vip-status ${dashboard.data.member.status}`}>{dashboard.data.member.status === 'active' ? 'VIP ativo' : dashboard.data.member.status === 'pending' ? 'Pagamento pendente' : dashboard.data.member.status}</span><span>{dashboard.data.subscription?.plan?.name || 'Plano em processamento'}</span></div>{dashboard.data.balance && <div className="vip-balance-grid"><div><strong>{dashboard.data.balance.cut}</strong><span>cortes disponíveis</span></div><div><strong>{dashboard.data.balance.beard}</strong><span>barbas disponíveis</span></div><div><strong>{dashboard.data.balance.eyebrow}</strong><span>sobrancelha disponível</span></div></div>}{dashboard.data.cycle ? <p className="club-form-note">Ciclo atual válido até {new Date(dashboard.data.cycle.endsAt).toLocaleDateString('pt-BR')}.</p> : <p className="club-form-note">Seu cadastro VIP existe, mas o ciclo só aparece após o pagamento ser aprovado.</p>}<div className="vip-history"><h3>Histórico de consumo</h3>{dashboard.data.usage?.length ? dashboard.data.usage.map((item, index) => <span key={`${item.kind}-${index}`}><Check size={14} /> {item.quantity} · {item.kind}</span>) : <p>Nenhum benefício consumido neste ciclo.</p>}</div>{dashboard.data.cycle?.status === 'active' && <button className="primary roulette-button" type="button" onClick={doSpin} disabled={spin.isPending}>{spin.isPending ? 'Girando…' : 'Girar roleta do ciclo'} <Sparkles size={16} /></button>}</> : <div className="club-placeholder"><Sparkles size={21} /><span>Você ainda não possui um cadastro VIP ativo</span><small>Escolha um plano para iniciar sua solicitação.</small></div>}</section>}
    <section id="beneficios" className="club-benefits-grid"><article><p className="eyebrow">BENEFÍCIOS</p><h2>Presença que<br /><em>volta para você.</em></h2><p>Cortes e benefícios controlados por ciclo.</p></article><article><p className="eyebrow">PARCEIROS</p><h2>Vantagens<br /><em>fora da cadeira.</em></h2>{partners.data?.length ? partners.data.map((partner) => <div className="partner-row" key={partner.id}><strong>{partner.name}</strong><span>{partner.benefits?.map((benefit) => benefit.title).join(' · ') || partner.description || 'Benefício em breve'}</span></div>) : <p className="club-empty-note">Os parceiros serão exibidos assim que forem cadastrados pela barbearia.</p>}</article><article><p className="eyebrow">SORTEIO MENSAL</p><h2>Uma campanha.<br /><em>Um vencedor.</em></h2>{raffles.data?.length ? raffles.data.map((raffle) => <div className="raffle-row" key={raffle.id}><strong>{raffle.prize}</strong><span>{raffle.campaign} · sorteio em {new Date(raffle.drawAt).toLocaleDateString('pt-BR')}</span>{mode === 'vip' && <button type="button" className="raffle-join" onClick={() => joinRaffle.mutate({ raffleId: raffle.id }, { onSuccess: () => setFeedback('Você entrou nesta campanha mensal.'), onError: (error) => setFeedback(error.message) })} disabled={joinRaffle.isPending}>Participar do sorteio</button>}</div>) : <p className="club-empty-note">A próxima campanha mensal será publicada aqui.</p>}</article></section><InternalLink className="secondary light-secondary" href="/planos"><ArrowRight size={17} /> Voltar para os planos</InternalLink><InternalLink className="club-final-link" href="/#inicio">Voltar ao site principal <ArrowUpRight size={15} /></InternalLink></main></div>
}
